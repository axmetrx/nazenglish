...
function VideoPlayerFrame({ url, title }) {
  ...
  // - Mobile devices (iPhone, iPad, Android) natively support mobile-recorded videos without desktop iframe toolbar clutter.
  // - Laptops / PCs use the universal Google Drive preview embed to ensure flawless H.264 playback without black screen.
  const [useNativePlayer, setUseNativePlayer] = useState(() => {
  ...
  const isDirectStream = Boolean(useNativePlayer && streamUrl);

  return (
    <div
      className={`yt-player-container ${isFullscreen ? 'is-fullscreen' : ''}`}
      onMouseMove={isDirectStream ? triggerShowControls : undefined}
      onClick={isDirectStream ? triggerShowControls : undefined}
    >
      {isDirectStream ? (
        /* Native Mobile HTML5 Video Stream (Clean mobile UI, no iframe toolbars) */
        <video
          ref={videoRef}
          src={streamUrl}
          playsInline
          ...
          onError={() => setUseNativePlayer(false)}
        ...
        /* Universal H.264 Embed Player for Laptop/Desktop (Flawless picture + sound) */
        <iframe
          src={embedUrl}
          title={title || 'Видео сабак'}
      ...
      {/* YouTube Mobile UI Controls Overlay (Active on phones for native stream) */}
      {isDirectStream && (
        <div
